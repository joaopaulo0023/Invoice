...
# Configurações
# -------------------------------
BASE_PDF_DIR = r"E:\Pedidos"
SAVE_DIR     = r"E:\Pedidos_Novos"
...
@st.cache_data(show_spinner=False)
def extrair_dados_pdf(order_number: str):
    order = order_number.strip()
    pdf_path = os.path.join(BASE_PDF_DIR, f"{order}.pdf")
    ...
    try:
        ...
        # se falhar já retorna vazio
        return {}, []

    # 2) Extrai header (lista, data, pedido)
    h = re.search(
        r"Lista de Separacão:\s*(\d+)\s+Data do Pedido:\s*(\d{2}/\d{2}/\d{2})\s+N\.\u00ba Pedido:\s*(\d+)",
        text
    )
    if h:
        ...
        lista, data = '', ''
        m = re.search(r"N[úu]mero do Pedido[:\s]*(\d+)", text)
        pedido = m.group(1) if m else order
    header = {'lista': lista, 'data': data, 'pedido': pedido}

    # 3) Quebra em linhas e tenta extrair itens via parsing de colunas
    lines = text.splitlines()
    items_dicts = []
    start = next((i for i, L in enumerate(lines) if 'Código do Item' in L), None)
    if start is not None:
        for L in lines[start+1:]:
            ...
            if len(parts) < 2:
                continue
            raw = parts[0].strip()
            m_code = re.match(r"(\d{9}[A-Z]{3})", raw)
            code = m_code.group(1) if m_code else raw.split()[0]
            m_qty = re.search(r"\d+", parts[1])
            if m_qty:
                items_dicts.append({
                    'item': code,
                    'pick_qty': int(m_qty.group())
                })

    # 4) Se nada foi encontrado, usa fallback genérico por regex no texto inteiro
    if not items_dicts:
        fb = re.findall(r"^(\d{9}[A-Z]{3})", text, re.MULTILINE)
        cnt = re.findall(r"\(\s*(\d+)\s*\)", text, re.MULTILINE)
        items_dicts = [
            {'item': code, 'pick_qty': int(qty)}
            for code, qty in zip(fb, cnt)
        ]

    # 5) Converte lista de dicts em lista de tuplas (disp, pdf_ref)
    items = []
    for d in items_dicts:
        disp    = f"{d['item']} ({d['pick_qty']})"
        pdf_ref = d['item']
...
with st.sidebar.expander("Referências", expanded=True):
    for idx_item, (disp, pdf_ref, tags) in enumerate(items_raw):
        m = re.search(r"\((\d+)\)", disp)                             #kkkkkk
        original = int(m.group(1)) if m else 0
        picked = sum(
            int(re.search(r"\((\d+)\)", t).group(1))
            for t in tags
            if re.search(r"\((\d+)\)", t)
        )
        missing = original - picked
        status = f"🚧 Faltam {missing}" if missing > 0 else "✅ Completo"
...
def processar_qr():
    ...
    if m:
        ...
        idx_item = next(
            (j for j, (_, pdf_ref_j, _) in enumerate(items)
        ...
        if idx_item is None:
            ...
            # --- calcula original e somado ---
            m0 = re.search(r"\((\d+)\)", disp_sel)
            orig = int(m0.group(1)) if m0 else 0
            somado = sum(
                int(re.search(r"\((\d+)\)", t).group(1))
                for t in tags_sel
                if re.search(r"\((\d+)\)", t)
            ...
            if somado >= orig:
                feedback = "✅ Quantidade já atingida — não é possível adicionar mais picks."
            else:
...
# ----------------------
# Se houver referência selecionada via sidebar, exibe detalhes na área principal      kkkkk
# ----------------------
...
if sel_idx is not None:
    ...
    missing = original - picked
    if missing > 0:
        st.info(f"🚧 Quantidade faltante: {missing}")
    else:
        st.success("✅ Pick completo para este item!")
    st.markdown("---")
    ...
    # Só permite adicionar se ainda faltar qty
    if missing > 0:
        if st.button("➕ Adicionar Etiqueta/Quantidade", key=f"btn_add_sel_{sel_idx}"):
            tags.append("")
    else:
        st.info("✅ Este item já está completo – não é possível adicionar mais etiquetas.")
